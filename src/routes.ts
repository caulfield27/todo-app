export const apiUrl = {
  ...
  getCompletedTodoes: (id: number | string, day: string) =>
    `/todoes?populate=*&filters[userId][id]=${id}&filters[isCompleted]=true`,
  getImportantTodoes: (id: number | string, dat: string) =>
    `/todoes?populate=*&filters[isCompleted]=false&filters[priority][$gt]=2`,
  getUpcomingTodoes: (id: number | string, today: string) =>
    `/todoes?populate=*&filters[userId][id]=${id}&filters[deadline][$gt]=${today}&filters[isCompleted]=false`,
